...
@register_model('transformerkey_mass')
class TransformerKeyMASSModel(FairseqEncoderDecoderModel):
    """
    Args:
        encoder (TransformerEncoder): the encoder
        decoder (TransformerDecoder): the decoder
    ...
    def build_model(cls, args, task):
        ...
        encoder = TransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = TransformerKeyDecoder(args, tgt_dict, decoder_embed_tokens)

        model = TransformerKeyMASSModel(encoder, decoder)
        ...
        return TransformerKeyMASSModel(encoder, decoder)

    def max_positions(self):
        return (self.encoder.max_positions(), self.decoder.max_positions())

    def forward(self, src_tokens=None, src_lengths=None, keyword_score=None, prev_output_tokens=None, **kwargs):
        """
        Run the forward pass for an encoder-decoder model.
        ...
        """
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, **kwargs)
        decoder_out = self.decoder(prev_output_tokens, encoder_out=encoder_out, keyword_score=keyword_score, **kwargs)
        return decoder_out
...
class TransformerKeyDecoderLayer(nn.Module):
    def __init__(
        self,
        embedding_dim: float = 768,
        ffn_embedding_dim: float = 3072,
        ...
        self.keyword_attn = MultiheadAttention(
            self.embedding_dim,
            num_attention_heads,
            kdim=embedding_dim,
            vdim=embedding_dim,
        ...
        self.keyword_attn_layer_norm = LayerNorm(self.embedding_dim, export=export)
    ...
    def forward(
        ...
        residual = x
        if prev_key_attn_state is not None:
            if incremental_state is None:
                incremental_state = {}
            prev_key, prev_value = prev_key_attn_state
            saved_state = {"prev_key": prev_key, "prev_value": prev_value}
            self.encoder_attn._set_input_buffer(incremental_state, saved_state)
        keyword_score = keyword_score + encoder_out
        x, attn = self.keyword_attn(
            query=x,
            key=keyword_score,
            value=keyword_score,
            key_padding_mask=encoder_mask,
        ...
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        x = self.keyword_attn_layer_norm(x)
...
class TransformerKeyDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.keyword_fc = nn.Linear(1, args.decoder_embed_dim)
    ...
    def forward(self,
                ...
                incremental_state=None, 
                **unused):
        x, extra = self.extract_features(prev_output_tokens, encoder_out, keyword_score, incremental_state, **unused)
        x = self.output_layer(x)
        return x, extra

    def extract_features(self, prev_output_tokens, encoder_out=None, keyword_score=None, incremental_state=None, **unused):
        # embed positions
        ...
        if keyword_score is not None:
            keyword_score = self.keyword_fc(keyword_score.transpose(0, 1).unsqueeze(-1))
        ...
        for layer in self.layers:
            x, attn = layer(
                ...
                keyword_score if keyword_score is not None else None,
                incremental_state,
...
@register_model_architecture('transformerkey_mass', 'transformerkey_mass')
def basekey_architecture(args):
...
@register_model_architecture('transformerkey_mass', 'transformerkey_mass_base')
